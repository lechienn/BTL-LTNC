...
public:
    ...
    void move(Uint32 &time, bool &player1Turn,bool &isFire, GameObject *player, GameObject *enermy, bool &increaseDamageTrigger, bool &doubleShootTrigger);
    void update();
    void render();
    void renderWinSpeed();
};
...
void Weapon::move(Uint32 &time, bool &player1Turn, bool &isFire, GameObject *player, GameObject *enermy, bool &increaseDamageTrigger, bool &doubleShootTrigger){
    ...
    int damage = 25;
    if(increaseDamageTrigger) {damage = 40;}
    ...
    if(xPos >= SCREEN_WIDTH || xPos <= 0 || yPos >= 600){
        isFire = false;
        if(!doubleShootTrigger) player1Turn = !player1Turn;
        player->setPower(0);
        // enermy->setPower(0);
        player->setFrame(0);
        enermy->setFrame(4);
        ...
        if(increaseDamageTrigger) {increaseDamageTrigger = false;}
        if(doubleShootTrigger) {doubleShootTrigger = false;}
        winSpeed = rand() % maxWinSpeed /10.0 * ((rand() % 2 == 1)?1.0:-1.0);
    }

    //if weapon hit // 64 = width of character img
